<?php
    ...
    if(isSecure()){
        ...
        $query = "SELECT * FROM infusion_pumps WHERE created_at LIKE '$today%' AND confinement_id = '$confinementId'";
        ...
        if($infusionPumpRow = $infusionPump->fetch_assoc()){
            ...
            $query = "INSERT INTO infusion_pumps(`confinement_id`,`tick`,`price`,`created_at`,`updated_at`)VALUES('$confinementId','1','$price','$todayTimeStamp','$todayTimeStamp')";
            $con->query($query) or die($con->error);
            $query = "SELECT * FROM infusion_pumps WHERE id = LAST_INSERT_ID()";
